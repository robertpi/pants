...
Fixed a `ClasspathSourceAmbiguity` engine error raised when a `protobuf_sources`/`protobuf_source` target was consumed by both the Java and Scala protobuf codegen backends (i.e. both `java_sources` and `scala_sources` targets depended on it). A new `jvm_codegen_type` field on protobuf targets lets a single declaration serve both languages via `parametrize`:
