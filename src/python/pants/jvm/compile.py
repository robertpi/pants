...
class ClasspathEntryRequestFactory:
    ...
    # `GenerateSourcesRequest`s may map to the same impl while sharing the same `input` field --
    # e.g. both the Java and Scala protobuf codegen backends consume `ProtobufSourceField`. In
    # that case, `GenerateSourcesRequest.codegen_variant_field`/`.codegen_variant` let a target
    # disambiguate between them (see `classify_impl`), so that a target isn't treated as
    # compatible with more than one of them at once (which would otherwise raise
    ...
    def classify_impl(
        ...
        def is_wrong_variant(target: Target, generator: type[GenerateSourcesRequest]) -> bool:
            field = generator.codegen_variant_field
            # `target.get()` returns the field's default if it isn't registered on this target
            # type, so it's safe to call unconditionally once we know the field is registered.
            if field is None or generator.codegen_variant is None or not target.has_field(field):
                return False
            value = target.get(field).value
            return value is not None and value != generator.codegen_variant
        ...
        def is_compatible(target: Target) -> bool:
            return (
                ...
                any(
                    target.has_field(generator.input) and not is_wrong_variant(target, generator)
                ...
                (
                    ...
                    and any(
                        ...
                        and not is_wrong_variant(target, generator)
                        for generator in generators
                    )
                )
            )
